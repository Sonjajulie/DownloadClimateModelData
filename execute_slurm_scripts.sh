...
foreach var ($arr) 
    sbatch --output=output_$var.out --error=error_$var.err --job-name=cesm_$var download_CESM_data.sh $var "cam.h0" atm
end

# fraction of ground covered by snow 
# not in for loop included because stream and component are different
set var = fsno
sbatch --output=output_$var.out --error=error_$var.err --job-name=cesm_$var download_CESM_data.sh $var "clm2.h0" lnd

# Geopotential Z at 500 mbar pressure surface (Z500)
# not in for loop included because stream and shell script are different
set var = Z500
sbatch --output=output_$var.out --error=error_$var.err --job-name=cesm_$var download_CESM_data_daily.sh $var "cam.h1" atm 
