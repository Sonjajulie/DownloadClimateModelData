...
set file_path_in=/gpfs/fs1/collections/cdg/data/cesmLE/CESM-CAM5-BGC-LE/$component/proc/tseries/daily/$var/
...
foreach i (`seq 1 1 35`)
    ...
    if ($i == 1) then
        set year_start = 18500101
        set year_start_final = 185001
        set year_mid = 20801231
    else if ($i >33) then
        set year_start = 19200101
        set year_start_final = 192001
        set year_mid = 21001231
    else
        set year_start = 19200101
        set year_mid = 20801231
    endif
    ...
    set file1 = $file_path_out/$var_out.$pad_i.$year_start-20051231.nc
    set file2 = $file_path_out/$var_out.$pad_i.20060101-$year_mid.nc
    # select only variable psl ~ Sea level pressure
    cdo selvar,$var $file_path_in/b.e11.B20TRC5CNBDRD.f09_g16.$pad_i.$stream.$var.$year_start-20051231.nc $file1_t
    cdo selvar,$var $file_path_in/b.e11.BRCP85C5CNBDRD.f09_g16.$pad_i.$stream.$var.20060101-$year_mid.nc $file2_t
    ...
    if ($i < 34) then
        ...
        set file3 = $file_path_out/icefrac.$pad_i.20810101-21001231.nc
        set file3_t = $file_path_out/fsno.$pad_i.3.nc
        cdo selvar,$var $file_path_in/b.e11.BRCP85C5CNBDRD.f09_g16.$pad_i.$stream.$var.20810101-21001231.nc $file3_t
        cdo seasmean -selmon,9,10,11 $file3_t $file3
        # merge to one file 
        cdo mergetime $file1 $file2 $file3 $file_path_out/$var_out.$pad_i.$year_start_final-210012.nc
        # remove other files
